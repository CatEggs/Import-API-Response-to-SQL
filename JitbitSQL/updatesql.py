...
def get_fields(tix_response):
    try:
        ...
        lastupdate = str(pd.to_datetime(tix_response['LastUpdated']).strftime("%Y-%m-%d %H:%M:%S")) if status != "New" else str((pd.to_datetime(tix_response['LastUpdated']) - timedelta(seconds=1)).strftime("%Y-%m-%d %H:%M:%S"))
        print(lastupdate)
        print(tix_response['LastUpdated'])
